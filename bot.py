...
# Import admin functions at the top
from admin import (
    admin_panel, handle_admin_action, approve_deposit, approve_withdrawal,
    update_profit, update_crypto_address, approve_user, list_users, admin_help,
    send_admin_panel, user_data, crypto_addresses
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_id == admin_id:
        await update.message.reply_text("Use /adminpanel to access admin features.")
...
async def start_registration(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_id == admin_id:
        await context.bot.send_message(
            ...
            text="Admins cannot register as users."
        )
        return ConversationHandler.END
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_id == admin_id:
        await update.message.reply_text("Admins cannot register as users.")
        return ConversationHandler.END
...
Please create an account for this user on novacapitalwealthpro.com and send them the login details."""
    ...
    keyboard = [[InlineKeyboardButton("✅ Approve", callback_data=f'approve_user_{user_id}')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    await update.message.reply_text(
        "Your registration is awaiting admin confirmation. You'll be notified once approved.",
        reply_markup=create_cancel_keyboard()
    )
    return MAIN_MENU
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_id == admin_id:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Use /adminpanel to access admin features."
        )
        return ConversationHandler.END
...
async def get_deposit_proof(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Get deposit proof screenshot and notify admin"""
    user_id = update.effective_user.id
    admin_id = get_admin_id()
    user_info = get_user_data(user_id)
...
Click 'Approve' to confirm this deposit."""
    ...
    keyboard = [[InlineKeyboardButton("✅ Approve", callback_data=f'confirm_deposit_{user_id}_{amount}')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
👤 **User:** {user_info['name']} (ID: {update.effective_user.id})
💰 **Amount:** ${amount:.2f}
...
Click 'Approve' to confirm this withdrawal."""
    ...
    keyboard = [[InlineKeyboardButton("✅ Approve", callback_data=f'approve_withdrawal_{update.effective_user.id}_{amount}')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def get_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id == admin_id:
        await update.message.reply_text(f"Your Admin ID is: {user_id}")
    else:
...
def main() -> None:
    ...
    # Store admin_id in bot_data for easy access
    admin_id = get_admin_id()
    if admin_id:
        ...
        logger.error("ADMIN_USER_ID not set! Admin features will not work.")
    
    # Add admin handlers first
    ...
    application.add_handler(CallbackQueryHandler(approve_user_button, pattern='^approve_user_'))
